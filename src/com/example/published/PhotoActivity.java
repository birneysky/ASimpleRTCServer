...
public class PhotoActivity extends Activity
{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		photo_bt_exit.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) { // 退出图片预览
				finish();
			}
		});
		photo_bt_del = (Button) findViewById(R.id.photo_bt_del);
		photo_bt_del.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) { // 删除当前图片
				if (listViews.size() == 1) {
					Bimp.bmp.clear();
					Bimp.drr.clear();
					Bimp.max = 0;
		...
		photo_bt_enter.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) { // 什么作用？
				Bimp.bmp = bmp;
				Bimp.drr = drr;
				Bimp.max = max;
		...
		if(type!=null && "look".equals(type)){
			unDisplayBtn();
		}
		pager.setCurrentItem(id);
	}

	private void unDisplayBtn() {
